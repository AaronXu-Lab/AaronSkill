...
将多弹层流程放在同一可操作画布中，直接审阅组件、状态和分支。主流程负责业务设计；初始化模块只负责可复用承载环境，不在每次业务迭代时重建画布。

## 执行入口

先读 [执行工作流](docs/workflow.md)，按任务选择资料：

| 当前任务 | 读取入口 |
| --- | --- |
| 只搭建环境，或发现现有能力缺失/失效 | [独立初始化模块](references/initialization.md)：检测复用、依赖与路由、隔离、操作和连线；产出就绪记录 |
