...
7. **按证据决定真实占位范围提示。** 当组件自身边界不明显，且真实占位范围对当前设计审阅有必要时，Gallery 才补充范围提示；判断依据是边界可辨识度与审阅必要性，与组件类别无关。优先复用目标项目既有的 Dashed Light / line-light；提示必须保持 Gallery-only，不得改变组件布局、冒充公开 API 或出现在 Caption 中。边界已清晰或无范围审阅价值时不机械添加。
...
浮层、菜单、Popover 等表面应直接展示其表面本身；只有触发方式属于组件公开契约且是审阅目标时，才把触发器纳入对比。保持语义排序、尺寸边界、键盘与焦点等可访问性检查。视觉验证默认只检查 Light；组件正确使用主题 color token 且不存在主题专属分支或已知主题风险时，不重复验证 Dark。只有这些条件不成立、用户明确要求或视觉证据表明 Dark 可能异常时，才追加 Dark 验证。仍按组件需要检查视口、窄容器、溢出和响应式条件。
