---
name: aw-meta-skill
description: 基于 skill-creator 创建或更新 Codex SKILL，并额外强制维护 docs/workflow.svg，以及包含 version、author、creation_context 的 metadata。用户要求新建、改造、升级或规范化 SKILL，且需要这些 AW 交付约束时使用。
metadata:
  version: "2.0.0"
  author: "aaron_xu"
  creation_context: "为统一 AW 系列 SKILL 的创建与更新流程，确保每个交付都包含可维护的阶段流程图和可追溯的版本、作者及创建语境元数据而创建。"
---
...
2. `docs/workflow.svg` 存在，并准确展示该 SKILL 的主要阶段、关键判断、停止条件和最终产物。
3. `SKILL.md` 正文使用 `![流程图说明](docs/workflow.svg)` 引用流程图。
4. 创建和更新完成后，同时运行基础校验器和本 SKILL 的附加校验器。
...
完整读取 `$skill-creator`。若为更新，先检查目标 SKILL 的 `SKILL.md`、`agents/openai.yaml`、`docs/workflow.svg`、脚本、引用资料及调用关系；保留不在用户范围内的现有字段和资源，不重新初始化已有目录。
...
每次创建或更新都检查 `docs/workflow.svg`，缺失则创建，流程变化则同步修改。SVG 必须：
...
更新时以当前 `SKILL.md` 的真实工作流为准。删除、合并或新增阶段后，正文与 SVG 必须一致；不要仅为了通过校验保留过时节点。
...
附加校验只证明结构性不变量成立；仍需人工核对流程图是否真实反映阶段和判断、元数据是否准确、版本升级是否合理，以及基础 `skill-creator` 的内容质量要求是否满足。有脚本时运行其最小相关测试，有 UI metadata 时核对 `agents/openai.yaml` 与 SKILL 一致。

最终报告目标路径、版本变化、author/creation_context 的取值、流程图状态和全部校验结果。
