...
仅在创建、修改或视觉审查流程图时读取。是否需要复核及证据复用条件见 [执行工作流](../references/workflow.md)。
...
- 若存在 `workflow.{subname}.svg` 子图，主图对应阶段必须显示子图完整文件名；在支持时用指向子图的相对链接包裹该文件名。只显示“见子流程”或仅在 Markdown 中链接不合格。层级最多两层，子图不得再引用或拆出 workflow SVG；细节应在子图中展开，或拆成主图直接链接的同级子图；
