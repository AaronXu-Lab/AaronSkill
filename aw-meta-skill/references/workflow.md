...
本文是模型执行事实源，`../docs/workflow.svg` 是面向人的可视化投影。流程变化时，先根据用户需求与有效契约修订本文，再同步 SVG 和入口；过时的投影不能覆盖当前授权。
...
| SVG 创建、修改或视觉审查 | 读取 [SVG 规范](workflow-svg.md) 并查看相关图；其他任务无需加载视觉细则 |
...
所需工作流缺失时，在 `references/workflow.md` 补齐；已有内容仅同步受影响部分。它是模型读取的唯一执行事实源，独立描述阶段、关键判断、停止条件、异常路径和最终产物；不得在 `docs/` 留重复的 `workflow.md`。正文是规则事实源，不复制整份入口。
...
流程图层级最多两层：主流程 `docs/workflow.svg` → 子流程 `docs/workflow.{subname}.svg`。只有主流程仍难以清楚表达的复杂流程才拆子图。每个子图必须由主图中的对应阶段直接指向；该阶段须显示子图的完整可见文件名，并在 SVG 支持链接时提供指向子图的相对链接。不能只写“见子流程”，也不能只在 Markdown 中列出文件链接，因为静态预览器可能无法跳转。

子流程必须在第二层完整展开，不得继续引用或拆出 `workflow.*.svg` 子 workflow。若细节仍多，应在该子图内展开，或拆为同级子图并由主图相应阶段直接链接。不得递归下钻至第三层或更深。校验器检查主图对子图的可见文件名与链接，并拒绝子图再引用 workflow SVG；这些结构检查不替代对图中阶段语义和关联位置的人工复核。
