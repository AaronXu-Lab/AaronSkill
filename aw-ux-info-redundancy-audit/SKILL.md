...
先审计信息，再调整样式。把界面内容归一为事实，标出每个事实的所有出现位置，判断每处信息在当前阶段是否有必要价值、每次重复是否提供独立价值，并提出最小安全改动。首先把信息密度视为语义问题，其次才是布局问题。
...
完成当前状态盘点、信息价值与重复判断前，不得新增或修改 UI、组件、区块或数据展示。不要通过直接实现来探索界面需要什么。先完成审计输出，再实现审计明确支持的改动。
...
记录主要用户任务、进入语境、当前状态、可用操作、约束和恢复路径。判断信息价值前先区分空态、加载中、就绪、活跃、待处理、成功、错误、禁用和已完成等状态；不同状态可能有意表达不同信息。
...
对每个归一化事实，列出全部出现位置、界面位置和作用，并记录适用状态、版本、配置或语境，以及哪些后续阶段仍需保留。前一阶段有用不代表要贯穿流程；条件改变时，联动检查身份、描述、状态及操作是否仍适用。出现位置包括：
...
同时记录标题、图标、分割线和容器的识别、分组或边界作用；不承载事实的视觉元素也纳入必要性判断。

### 5. 分类重复信息

仅对存在重复的事实使用以下类别；唯一事实继续进入价值判断，不为分类虚构另一处表达：
...
界面包含多个状态、嵌套表面、重复摘要、条件或阶段变化，或需要区分装饰与操作入口时，读取 [references/redundancy-patterns.md](references/redundancy-patterns.md)。

### 6. 执行判断测试

对所有信息（包括只出现一次的事实）及视觉承载物执行判断：

1. 在适用条件和当前阶段，是否支持用户任务、对象识别、状态理解、决策、操作或恢复？若有重复，第二处是否回答了不同的用户问题？
...
9. 标题、图标、分割线或容器是否提供实际识别、分组或边界价值？文字已清楚表达操作时，图标不是必填项；没有实际分界时，不为形式完整追加标题或分隔。承担操作入口的图标须保留可发现性、可访问名称和操作能力。

作出以下决策：

- **保留**：承担独立任务、安全或恢复作用。
- **合并**：两处信息应该组成同一个决策单元。
- **删除**：同一语境中的其他位置已完整表达该事实，或有依据认定该信息在当前阶段无必要价值；视觉承载物不提供实际识别、分组或边界价值时也可删除。记录对应理由，不为唯一信息虚构重复来源。
- **缩短**：当前语境需要提示，但不需要再次解释。
- **移动**：事实有效，但远离它所支持的决策。
- **拆分**：一个区块混合状态、解释和操作，导致不同作用之间互相重复。

区分“已证实非必要”与“用途尚未理解”：后者标记待验证，不能据此删除。不要为了减少页面高度、窗口面积或屏幕占用而删除必要业务、安全或无障碍语义。对仍有价值的事实，保留最靠近决策或操作的标准位置；让次要位置更短、更贴合语境。

默认态降噪采用非对称披露：正常、最新、无异常且不可行动的状态通常不追加说明；偏离预期、存在风险或可行动时才增加原因、后果或入口。安全确认、审计要求或用户明确需要确认正常状态时除外。

删除状态文字或辅助信息前，确保仍然必要的信息有可感知、可理解的标准表达。不要让关键状态只依赖单一视觉、听觉、触觉、空间或动作线索；精简不能破坏目标界面的可访问性名称、状态语义，以及实际输入与辅助方式下的可达操作。
...
1. 删除已证实非必要的信息与视觉承载物，或合并重复文案和控件；
2. 集中标准状态、解释或操作；
...
- 保留信息具有当前阶段价值，重复表达有独立理由；删除的唯一信息有非必要性依据；
- 条件变化后身份、描述、状态和操作仍适用，后续需用的信息未被跨阶段误删；
- 移除视觉承载物后识别、分组、边界与关键状态仍清楚；
...
3. **信息原子映射**：归一化事实、界面位置、全部出现位置、适用条件和保留阶段，以及视觉承载物的作用。
...
9. **重复信息验证**：标准事实、合理的语境性重复、适用条件与阶段覆盖、视觉承载物价值、无障碍语义和待解决项。

信息价值与重复决策未明确前，不要把视觉改版描述为已经完成。
