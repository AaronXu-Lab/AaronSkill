...
多状态、跨表面、条件变化、摘要与详情，或装饰与操作难以区分时，按相关小节读取 [模式与示例](redundancy-patterns.md)。简单局部问题不默认加载整份参考，也不把示例当作通用布局模板。
...
实施前核对方案是否保留必要事实、适用条件、后续阶段和操作能力；发现矛盾或缺失时回到映射修正，方案推演不能写成实测通过。需要实施时再读取 [实施与验证](implementation.md)，完成相关修改、检查和范围内修复。
